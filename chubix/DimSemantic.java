...
*/
   ...
   @Override public Symbol visitRelativeDim(dimensionsParser.RelativeDimContext ctx) {
      ...
      if (ctx.ID(1)!=null) {
         ...
         DimensionsType newRelDim = new DimensionsType(dim, relDim.getUnit(), type); // add default
         newRelDim.addUnit(unit, 1.0); // add the new 

         dimensionsParser.dimTable.put(dim, newRelDim);  //add dim to map dimTable

         System.out.println("===== " + dim + " =========="+ newRelDim.getType().name());
         for (HashMap<String,Integer> m : newRelDim.getUnits().values())
            System.out.println(DimensionsType.mapToString(m));
      } else {
         DimensionsType newRelDim = new DimensionsType(dim, relDim.getUnit(), type); // add default unit generated by the the relationship of dims
   
         dimensionsParser.dimTable.put(dim, newRelDim);  // add dim to map dimTable 
         
         System.out.println("===== " + dim + " ========== type: " + newRelDim.getType().name());
         for (HashMap<String,Integer> m : newRelDim.getUnits().values())
            System.out.println(DimensionsType.mapToString(m));
      }
